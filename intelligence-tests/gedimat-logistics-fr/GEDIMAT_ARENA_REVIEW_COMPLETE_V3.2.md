...
**XCEL — Xpérience Client & Excellence Logistique**
...
**Le constat.** Aujourd’hui, il existe un vide fonctionnel entre la Vente (prise de commande) et l’Exploitation (stock, préparation, transport). Les incidents logistiques (retards, affrètements doublons, urgences mal gérées) sont traités au cas par cas par les commerciaux et le SAV, au détriment du temps de vente et de la lisibilité interne.

**Rôle XCEL – “tour de contrôle” client–logistique.** La fonction XCEL a pour mission de garantir que ce qui est vendu est livré comme promis, en jouant un rôle de chef d’orchestre entre les 3 dépôts, les transporteurs et les équipes commerciales.

**Missions principales :**
- **Pilotage des flux :** validation technique des commandes atypiques, choix du dépôt optimal selon la règle de proximité, arbitrage des priorités entre dépôts, coordination affrètement / navettes.
- **Suivi des incidents :** centralisation des alertes (ARC/ACK, J‑1, retards), préparation des décisions d’escalade, suivi des dossiers jusqu’à résolution.
- **Interface interne pour commerciaux/SAV :** point de contact unique pour les équipes commerciales et le SAV en cas de problème logistique, afin de limiter le temps passé hors vente (pas de front client direct, sauf escalades planifiées).
- **Traçabilité & indicateurs :** tenue d’un journal des dérogations, alimentation d’un tableau de bord simple (service, coûts, réclamations) pour la direction.

**Définition OTIF (On Time In Full).** OTIF = pourcentage de commandes B2B livrées à l’heure (fenêtre ≤ +30 minutes par rapport à l’horaire confirmé) et complètes (lignes livrées ≥ 99 %).

**SLA XCEL incidents courants.** Accusé de réception d’une alerte ≤ 30 minutes ouvrées, proposition de plan d’action ≤ 2 heures, résolution visée ≤ J+1 pour les incidents standard.

**Périmètre de décision XCEL.** Peut : arbitrer le dépôt d’expédition, séquencer les livraisons, proposer des substitutions produits équivalentes en les documentant. Ne peut pas : engager de remise tarifaire, modifier les CGV, promettre un créneau horaire hors grille sans validation du dépôt concerné.

**Sources de données minimales.** Disponibilité stock J0/J+1 par dépôt, planning camions/navettes, marquage des clients sensibles, gabarits produits.

**Profil requis.** Pour être opérationnel rapidement, ce poste requiert :
- Une compréhension fine des 3 dépôts (types de stocks, contraintes locales, habitudes de planification),
- Une connaissance des clients sensibles et de leurs contraintes de chantier,
- Une maîtrise opérationnelle des contraintes produits/transport (gabarits, arrimage, restrictions d’accès, ce qui rentre ou non dans un véhicule, contraintes de chargement).

**Délai d’intégration en cas de recrutement externe.** Pour un profil totalement externe, la montée en compétence sur ce type de poste est progressive :
- environ **3–6 mois** pour maîtriser la logistique de base des 3 dépôts (flux, types de produits, affrètements/navettes),
- environ **3 mois** pour comprendre les processus internes et les modes de décision,
- environ **3 mois** pour connaître les clients sensibles et devenir réellement proactif sur leurs besoins.

*S’il n’existe aucun profil interne correspondant à ce profil XCEL, cela implique un délai de montée en compétence estimé à **9–12 mois** avant que ce lien stratégique soit pleinement opérationnel. Pendant cette période d’apprentissage, la fonction ne pourra pas encore protéger à 100 % les commerciaux, le SAV, la comptabilité et les stocks des dépôts, ni jouer son rôle de relais fiable entre tous les acteurs.*
...
- **Utilité :** Guide quotidien pour la coordinatrice logistique
