...
Metaphor Shift:
  ...
  * Unit: Vesicle → Parcel (sealed container with tracking ID)
...
Core Philosophy: "No Schema, No Dispatch"
  Every Parcel is validated against a schema before it is handed to Redis.
  Every dispatch checks key type to avoid WRONGTYPE errors.
  Every container carries IF.TTT headers for auditability.

Responsibilities:
  1. Parcel - Dataclass representing a sealed container and its metadata.
...
Usage:
  from infrafabric.core.logistics.parcel import LogisticsDispatcher, Parcel
  ...
  dispatcher.dispatch_to_redis('queue:context', parcel)
...
class ParcelSchemaVersion(Enum):
    """Supported schema versions for parcel validation"""
...
class Parcel:
    ...
    Fields:
        ...
        contents: Arbitrary parcel contents (must serialize to msgpack)
        schema_version: Version of this parcel schema
    ...
    def __post_init__(self) -> None:
        """Validate parcel after initialization."""
    ...
    @classmethod
    def from_msgpack(cls, data: bytes) -> "Parcel":
...
class LogisticsDispatcher:
    ...
    def _validate_schema(self, parcel: Parcel) -> bool:
        """
        Validate parcel against schema.

        Implements: "No Schema, No Dispatch"

        Args:
            parcel: Parcel to validate
        ...
        schema = SCHEMA_MAP.get(parcel.schema_version)
        if not schema:
            raise ValueError(f"Unknown schema_version: {parcel.schema_version}")

        required = schema.get("required", [])
        parcel_dict = parcel.to_dict()

        for field_name in required:
            if field_name not in parcel_dict:
                raise ValueError(f"Missing required field: {field_name}")

        properties = cast(Dict[str, Any], schema.get("properties", {}))
        for field_name, field_value in parcel_dict.items():
    ...
    def dispatch_to_redis(
        ...
        """
        Dispatch parcel to Redis with schema validation and type checking.
        ...
        self._validate_schema(parcel)

        serialized: Any
        if use_msgpack:
            if not HAS_MSGPACK:
                raise RuntimeError("msgpack not installed. Install with: pip install msgpack")
            serialized = parcel.to_msgpack()
        else:
            serialized = parcel.to_json()
        ...
        try:
            if operation == "set":
                ...
                self.redis_client.set(key, serialized, ex=parcel.ttl_seconds)
                return True
            ...
            if operation == "lpush":
                ...
                self.redis_client.expire(key, parcel.ttl_seconds)
                return True
    ...
    def collect_from_redis(
        ...
        """
        Collect parcel(s) from Redis with automatic type checking and deserialization.
        ...
        Returns:
            Parcel or collection of Parcels, or None if key doesn't exist
        ...
        try:
            ...
            if operation == "get":
                ...
                return Parcel.from_msgpack(data) if use_msgpack else Parcel.from_json(data)
            ...
            if operation == "lrange":
                ...
                list_results: List[Parcel] = []
                for data in data_list:
                    parcel = Parcel.from_msgpack(data) if use_msgpack else Parcel.from_json(data)
                    list_results.append(parcel)
            ...
            if operation == "hgetall":
                ...
                dict_results: Dict[str, Parcel] = {}
                for field, data in data_dict.items():
                    parcel = Parcel.from_msgpack(data) if use_msgpack else Parcel.from_json(data)
                    dict_results[field] = parcel
            ...
            if operation == "smembers":
                ...
                set_results: List[Parcel] = []
                for data in data_set:
                    parcel = Parcel.from_msgpack(data) if use_msgpack else Parcel.from_json(data)
                    set_results.append(parcel)
...
class DispatchQueue:
    ...
    def add_parcel(self, key: str, parcel: Parcel, operation: str = "set") -> None:
        """Queue parcel for batch dispatch."""

        self.pending.append((key, parcel, operation))

    def flush(self) -> int:
        """Dispatch all queued parcels, return count."""

        count = 0
        for key, parcel, operation in self.pending:
            self.dispatcher.dispatch_to_redis(key, parcel, operation=operation)
            count += 1
        self.pending.clear()
        return count
...
# =============================================================================
# FLUENT INTERFACE: IF.Logistics
# =============================================================================

class _LogisticsFluentBuilder:
    """
    Fluent interface builder for civic logistics operations.

    Enables the human-readable pattern:
        IF.Logistics.dispatch(parcel).to("The Council")

    This is the civic equivalent of biological "vesicle transport" - parcels
    are dispatched through the logistics network to designated destinations.
    """

    def __init__(self, dispatcher: Optional[LogisticsDispatcher] = None):
        self._dispatcher = dispatcher
        self._parcel: Optional[Parcel] = None
        self._destination: Optional[str] = None
        self._operation: str = "set"

    def dispatch(self, parcel: Parcel) -> "_LogisticsFluentBuilder":
        """
        Begin a dispatch operation with the given parcel.

        Args:
            parcel: The Parcel to dispatch
    ...
    def to(self, destination: str, operation: str = "set") -> bool:
        """
        Complete the dispatch to a destination (Redis key).

        Args:
            destination: Redis key to dispatch to (e.g., "queue:council", "context:agent-42")
            operation: Redis operation type ("set", "lpush", "rpush", "hset", "sadd")

        Returns:
            True if dispatch succeeded

        Raises:
            ValueError: If no parcel was set or no dispatcher is configured
        """
        if self._parcel is None:
            raise ValueError("No parcel set. Call dispatch() first.")
        if self._dispatcher is None:
            raise ValueError("No dispatcher configured. Use IF.Logistics.connect() first.")

        self._destination = destination
        self._operation = operation

        return self._dispatcher.dispatch_to_redis(
            key=destination,
            parcel=self._parcel,
            operation=operation
        )

    def collect(self, source: str, operation: str = "get") -> Optional[Parcel]:
        """
        Collect a parcel from a source (Redis key).

        Args:
            source: Redis key to collect from
            operation: Redis operation type ("get", "lindex", "lrange", "hget", "hgetall", "smembers")

        Returns:
            Parcel or collection of Parcels, or None if not found

        Raises:
            ValueError: If no dispatcher is configured
        """
        if self._dispatcher is None:
            raise ValueError("No dispatcher configured. Use IF.Logistics.connect() first.")

        return self._dispatcher.collect_from_redis(key=source, operation=operation)


class _LogisticsNamespace:
    """
    IF.Logistics namespace providing civic logistics operations.

    Usage:
        # Initialize connection
        IF.Logistics.connect(host='localhost', port=6379)

        # Dispatch a parcel
        parcel = Parcel(origin='council-secretariat', contents={'query': 'find patterns'})
        IF.Logistics.dispatch(parcel).to("queue:council")

        # Collect a parcel
        result = IF.Logistics.collect("queue:council")

        # Or use the fluent builder directly
        IF.Logistics.dispatch(Parcel(...)).to("The Council")
    """

    _dispatcher: Optional[LogisticsDispatcher] = None
    ...
    def connect(
        ...
        **kwargs: Any
        ...
        """
        Connect to Redis and return the dispatcher.

        Args:
            host: Redis host
            port: Redis port
            db: Redis database number
            **kwargs: Additional arguments for LogisticsDispatcher

        Returns:
            LogisticsDispatcher instance
        """
        cls._dispatcher = LogisticsDispatcher(
            redis_host=host,
            redis_port=port,
            redis_db=db,
            **kwargs
    ...
    @classmethod
    def dispatch(cls, parcel: Parcel) -> _LogisticsFluentBuilder:
        """
        Begin a fluent dispatch operation.
        ...
        """
        builder = _LogisticsFluentBuilder(cls._dispatcher)
        return builder.dispatch(parcel)

    @classmethod
    def collect(cls, source: str, operation: str = "get") -> Optional[Parcel]:
        """
        Collect a parcel from a source.
        ...
        """
        builder = _LogisticsFluentBuilder(cls._dispatcher)
        return builder.collect(source, operation)

    @classmethod
    def get_dispatcher(cls) -> Optional[LogisticsDispatcher]:
...
class _IFNamespace:
    """
    Root IF namespace providing access to all InfraFabric subsystems.

    This is the civic entry point for the InfraFabric logistics layer.
    """

    Logistics = _LogisticsNamespace


# Singleton IF namespace
IF = _IFNamespace()
