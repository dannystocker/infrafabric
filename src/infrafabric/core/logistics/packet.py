...
class _LogisticsFluentBuilder:
    ...
    Enables the human-readable pattern:
        IF.Logistics.dispatch(packet).to("The Council")
    ...
    def __init__(self, dispatcher: Optional[LogisticsDispatcher] = None):
        ...
        self._parcel: Optional[Packet] = None
        self._destination: Optional[str] = None
        self._operation: str = "set"

    def dispatch(self, packet: Packet) -> "_LogisticsFluentBuilder":
        """
        Begin a dispatch operation with the given packet.
    ...
    def to(self, destination: str, operation: str = "set") -> bool:
        ...
        Raises:
            ValueError: If no packet was set or no dispatcher is configured
        """
        if self._parcel is None:
            raise ValueError("No packet set. Call dispatch() first.")
    ...
    def collect(self, source: str, operation: str = "get") -> Optional[Packet]:
        """
        Collect a packet from a source (Redis key).
        ...
        Returns:
            Packet or collection of Parcels, or None if not found
...
class _LogisticsNamespace:
    ...
    Usage:
        ...
        # Dispatch a packet
        packet = Packet(origin='council-secretariat', contents={'query': 'find patterns'})
    ...
    @classmethod
    def dispatch(cls, packet: Packet) -> _LogisticsFluentBuilder:
        """
        ...
        return builder.dispatch(packet)

    @classmethod
    def collect(cls, source: str, operation: str = "get") -> Optional[Packet]:
        """
        Collect a packet from a source.
