...
class SecretRedactorV3:
    ...
    PATTERNS = [
        ...
        # FTP config files (.ftpconfig) - includes passphrase
    ...
    def predecode_and_rescan(self, text: str) -> List[Tuple[str, str]]:
        ...
        Deduplicates by match text to avoid counting same secret multiple times.
        """
        results = []
        ...
        # Deduplicate by match text (keep first occurrence)
        seen_matches = set()
        deduplicated = []
...
if __name__ == "__main__":
    ...
    test_config = '''
    openai_api_key = "sk-test-example-key-1234567890"
    openai_api_endpoint = "https://api.openai.com/v1/chat/completions"
    '''

    api_key_pos = test_config.find("sk-test-example-key-1234567890")
    relationships = find_secret_relationships(
        "sk-test-example-key-1234567890",
        test_config,
        api_key_pos
    )
    conf_score = confucian_relationship_score(relationships)
    ...
    test_auth = '''
    authorization_token = "eyJhbGciExampleJWTTokenForTestingOnly"
    session_timeout = 3600
    bearer_auth_required = true
    '''

    token_pos = test_auth.find("eyJhbGciExampleJWTTokenForTestingOnly")
    relationships = find_secret_relationships(
        "eyJhbGciExampleJWTTokenForTestingOnly",
        test_auth,
        token_pos
    )
    conf_score = confucian_relationship_score(relationships)
